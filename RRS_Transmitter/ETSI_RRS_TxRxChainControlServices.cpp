...
bool ETSI_RRS_TxRxChainControlServices::tx_from_buff(std::vector<std::complex<float>> buff, size_t samps_per_buff) {
...
std::vector<std::complex<float>> ETSI_RRS_TxRxChainControlServices::rx_to_buff(size_t samps_per_buff, float settling_time, int num_requested_samps) {
    ...
    std::vector<std::complex<float>> buff(usrpDevice->rx_samps_per_buff); //было хорошо
