...
#include "receiver.h" // Было закоменчено
#include "model.h"
...
//void receiver(gsl_complex* time_signal,    // Было gsl_complex
...
//  }
  ...
  if ( !sim_params.idealFrequencySync )
  {
    ...
    rotor_state_angle = 0;
	  ...
	  rotor_11a( fxp_time_signal + start_sample,
						   ...
						   3 * NumOfSubcarriers + CyclicPrefixLength ); // вот тут страная ошибка
    //]
  }
  ...
  if ( num_of_OFDM_symbols * *bits_per_OFDM_symbol > MaxCodedPacketSize )
  {
    PLCP_error = -1;
    printf("PLCP_error = -1\n");
    printf("num_of_OFDM_symbols * *bits_per_OFDM_symbol = %d\n", num_of_OFDM_symbols * *bits_per_OFDM_symbol);
  }
